...
void setPressureBoundaryConditions() {
  ...
  for (int i = 0; i < leftNeighbourSize; i++) { // left neighbour
    p[leftNeighbour[i] - 1] = p[leftNeighbour[i]];
  }
  for (int i = 0; i < rightNeighbourSize; i++) { // right neighbour
    p[rightNeighbour[i] + 1] = p[rightNeighbour[i]];
  }
  for (int i = 0; i < bottomNeighbourSize; i++) { // bottom neighbour
    p[bottomNeighbour[i] - (numberOfCellsPerAxisX + 2)] = p[bottomNeighbour[i]];
  }
  for (int i = 0; i < topNeighbourSize; i++) { // top neighbour
    p[topNeighbour[i] + (numberOfCellsPerAxisX + 2)] = p[topNeighbour[i]];
  }
  for (int i = 0; i < frontNeighbourSize; i++) { // front neighbour
    p[frontNeighbour[i] - (numberOfCellsPerAxisX + 2) * (numberOfCellsPerAxisY + 2)] = p[frontNeighbour[i]];
  }
  for (int i = 0; i < behindNeighbourSize; i++) { // behind neighbour
    p[behindNeighbour[i] + (numberOfCellsPerAxisX + 2) * (numberOfCellsPerAxisY + 2)] = p[behindNeighbour[i]];
  }
}
...
void setupScenario() {
  ...
  indicesInDomainNonBoundarySize = indicesInDomainNonBoundaryIndex;
  indicesInDomainNonBoundary = new int[indicesInDomainNonBoundaryIndex];
  indicesInDomainNonBoundaryIndex = 0;
  ...
  leftNeighbour = new int[leftNeighbourIndex];
  rightNeighbour = new int[rightNeighbourIndex];
  bottomNeighbour = new int[bottomNeighbourIndex];
  topNeighbour = new int[topNeighbourIndex];
  frontNeighbour = new int[frontNeighbourIndex];
  behindNeighbour = new int[behindNeighbourIndex];
  ...
  leftNeighbourSize = leftNeighbourIndex;
  rightNeighbourSize = rightNeighbourIndex;
  bottomNeighbourSize = bottomNeighbourIndex;
  topNeighbourSize = topNeighbourIndex;
  frontNeighbourSize = frontNeighbourIndex;
  bottomNeighbourSize = bottomNeighbourIndex;
